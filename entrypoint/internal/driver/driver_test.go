...
var _ = Describe("Driver", func() {
	...
	Context("Build", func() {
		...
		It("should build driver successfully for Ubuntu with DKMS enabled", func() {
			...
			// Mock DKMS setup (called after installDriver when UseDKMS is true)
			...
			// Mock dkmsStatus - not installed
			cmdMock.EXPECT().RunCommand(ctx, "dkms", "status", "mlnx-ofa_kernel", "5.9.0.0.1.1.0").Return("", "", errors.New("not found"))
			// Mock dkmsAdd - check if already added
			cmdMock.EXPECT().RunCommand(ctx, "dkms", "status", "mlnx-ofa_kernel", "5.9.0.0.1.1.0").Return("", "", errors.New("not found"))
			// Mock dkmsAdd
			cmdMock.EXPECT().RunCommand(ctx, "dkms", "add", "-m", "mlnx-ofa_kernel", "-v", "5.9.0.0.1.1.0").Return("", "", nil)
			// Mock dkmsBuild - check if already built
			osMock.EXPECT().Stat("/var/lib/dkms/mlnx-ofa_kernel/5.9.0.0.1.1.0/5.4.0-42-generic").Return(nil, os.ErrNotExist)
			// Mock dkmsBuild
			cmdMock.EXPECT().RunCommand(ctx, "dkms", "build", "-m", "mlnx-ofa_kernel", "-v", "5.9.0.0.1.1.0", "-k", "5.4.0-42-generic").Return("", "", nil)
			// Mock dkmsInstall - check if already installed
			cmdMock.EXPECT().RunCommand(ctx, "dkms", "status", "mlnx-ofa_kernel", "5.9.0.0.1.1.0").Return("", "", errors.New("not found"))
			// Mock dkmsInstall
			cmdMock.EXPECT().RunCommand(ctx, "dkms", "install", "-m", "mlnx-ofa_kernel", "-v", "5.9.0.0.1.1.0", "-k", "5.4.0-42-generic").Return("", "", nil)
	...
	Context("Load", func() {
		...
		It("should setup DKMS when UseDKMS is enabled and modules match", func() {
			...
			// Mock DKMS setup (called in Load when UseDKMS is true)
			hostMock.EXPECT().GetKernelVersion(ctx).Return("5.4.0-42-generic", nil)
			// Mock discoverDKMSModule - ReadDir /usr/src/
			mockEntry := mockDirEntry{name: "mlnx-ofa_kernel-5.9-5.9.0.0.1.1.0", isDir: true}
			osMock.EXPECT().ReadDir("/usr/src/").Return([]os.DirEntry{mockEntry}, nil)
